...
ACCOUNT_SIZE        (opsiyonel variable, varsayılan 60 — $ cinsinden alım bütçesi)
...
ACCOUNT_SIZE = _env_float("ACCOUNT_SIZE", 60.0)          # alım bütçen ($)
...
def position_size(entry, stop):
    """İşlem başı risk (komisyon dahil) bütçenin RISK_PER_TRADE_PCT'sini aşmasın.
    Komisyon tek başına bu sınırı aşıyorsa (küçük bütçe) 1% kuralı uygulanamaz;
    o zaman komisyonun payı en küçük olsun diye bütçenin tamamı kullanılır."""
    per_share = entry - stop
    if per_share <= 0 or entry <= 0:
        return 0, 0.0
    by_budget = int(ACCOUNT_SIZE // entry)
    risk_money = ACCOUNT_SIZE * RISK_PER_TRADE_PCT / 100 - ROUND_TRIP_FEES
    if ROUND_TRIP_FEES > 0 and risk_money <= 0:
        shares = by_budget
    else:
        shares = min(by_budget, int(risk_money // per_share))
...
def backtest_setup(df, daily_atr_pct, min_reward=MIN_REWARD_PCT, fee_pct=0.0):
    ...
    2 saatlik tepenin kapanışla kırılması) bu hissede son 60 günde oluştuğunda
    aynı giriş/stop/hedef kurallarıyla ne olurdu? Yaklaşık bir simülasyondur."""
    ...
    if not trades:
        return {"n": 0}
    pnls = [p - fee_pct for _, p in trades]          # komisyon sonrası net %
    counts = {k: sum(1 for r, _ in trades if r == k) for k in ("target", "stop", "breakeven", "eod")}
