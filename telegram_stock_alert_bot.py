...
MIN_DAILY_VOLATILITY_PCT = 3.5      # HARD FLOOR: daily ATR/price %% below this = too calm, excluded
                                      # entirely (this is what filters out AAPL/GOOGL-style mega-caps
                                      # that rarely move enough to hit a 5%% profit target)
RANK_VOLATILITY_SWEET_SPOT = 5.0    # among candidates that clear the floor, ATR/price %% that scores highest
RANK_VOLATILITY_TOLERANCE = 4.0     # how quickly the score falls off away from the sweet spot
...
MIN_REWARD_PCT = 5.0                  # HARD FLOOR: any alert whose computed target is < 5%% away
                                       # from entry gets discarded, no matter which stock it is
...
def rank_candidates():
    ...
    for t in CANDIDATE_UNIVERSE:
        try:
            ...
            # HARD FLOOR: too calm to realistically produce a 5%+ intraday move —
            # exclude entirely, regardless of how clean its trend/proximity look.
            if volatility_pct < MIN_DAILY_VOLATILITY_PCT:
                log.info("%s: günlük oynaklık %%%.1f, %%%.1f eşiğinin altında — elendi (çok sakin).",
                          t, volatility_pct, MIN_DAILY_VOLATILITY_PCT)
                continue
...
def evaluate(ticker: str, quote: dict, session: str):
    ...
    # HARD FLOOR: if this setup can't realistically offer at least MIN_REWARD_PCT,
    # skip it — no matter which stock it is. This is what stops the bot from
    # alerting on technically-clean-but-tiny moves (the AAPL/GOOGL problem).
    if reward_pct < MIN_REWARD_PCT:
        log.info(
            "%s: skor %d/5 ve kırılmaya yakın AMA potansiyel kâr %%%.2f < %%%.1f eşiği — elendi.",
            ticker, score, reward_pct, MIN_REWARD_PCT,
        )
        return None
